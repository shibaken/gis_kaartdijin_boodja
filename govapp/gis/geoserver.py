...
class GeoServer:
    ...
    @handle_http_exceptions(log)
    def create_store_if_not_exists(self, workspace_name, store_name, data, store_type='datastores'):
        # URL to check the existence of the store
        store_get_url = f"{self.service_url}/rest/workspaces/{workspace_name}/{store_type}/{store_name}"
        ...
        # Construct URL
        url = f"{self.service_url}/rest/workspaces/{workspace_name}/{store_type}"
    ...
    def upload_store_wms(self, workspace, store_name, context) -> None:
        ...
        response = self.create_store_if_not_exists(workspace, store_name, data, store_type='wmsstores')
