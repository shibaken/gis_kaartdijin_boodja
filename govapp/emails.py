...
def sendHtmlEmail(to,subject,context,template,cc,bcc,from_email,template_group,attachments=None):
    ...
    if template_group == 'system-oim':
        main_template = get_template('email/base_email-oim.html').render(context)
    elif template_group == 'pvs': 
        with open(str(settings.BASE_DIR)+'/govapp/static/govapp/img/pvs_logo.gif','rb') as f:
            logomime = MIMEBase('image','png', filename='Logo')
            logomime.add_header('Content-Disposition', 'attachment', filename='pvs.png')
            logomime.add_header('X-Attachment-Id', 'pvs_logo')
            logomime.add_header('Content-ID', '<pvs_logo>')
            logomime.set_payload(f.read())
            encoders.encode_base64(logomime)
            #parkstay/static/ps/img/parkstay_bookings_logo.png

        main_template = get_template('email/base_email-pvs.html').render(context)
    else:
        main_template = get_template('email/base_email.html').render(context)
